...
async function ensureDay(docRef, data) {
  ...
  if (data.weekKey !== currentWeekKey) {
    ...
    updates.invitesCompletedLastWeek = data.invitesCompletedThisWeek || 0;
    updates.invitesCompletedThisWeek = 0;
    updates.invitesStartedLastWeek = data.invitesStartedThisWeek || 0;
    updates.invitesStartedThisWeek = 0;
    updates.commissionLastWeek = data.commissionThisWeek || 0;
    updates.commissionThisWeek = 0;
    needsUpdate = true;
  }
  if (needsUpdate) {
...
<body>
      ...
      <table id="usersTable" style="display:none;">
        <thead>
          <tr>
            ...
            <th>Last Week</th>
            <th>Invites Done</th>
            <th>Invites This Wk</th>
            <th>Invites Last Wk</th>
            <th>Invites Started</th>
            <th>Started This Wk</th>
            <th>From Invites</th>
            <th>From Commission</th>
            <th>Commission This Wk</th>
            <th>Commission Last Wk</th>
          </tr>
        </thead>
  ...
  <script>
    ...
    function displayUsers(users) {
      ...
      users.forEach(user => {
        ...
        row.innerHTML = \`
          ...
          <td>\${user.invitesCompletedThisWeek || 0}</td>
          <td>\${user.invitesCompletedLastWeek || 0}</td>
          <td>\${user.invitesStarted || 0}</td>
          <td>\${user.invitesStartedThisWeek || 0}</td>
          <td>\${user.tokensFromInvites || 0}</td>
          <td>\${user.tokensFromCommission || 0}</td>
          <td>\${user.commissionThisWeek || 0}</td>
          <td>\${user.commissionLastWeek || 0}</td>
        \`;
    ...
    function exportCSV() {
      const csv = [
        ['Email', 'Email Verified', 'Wallet', 'Referral Code', 'Invited By', 'Tokens Total', 'From Ads', 'Ads Watched', 'This Week', 'Last Week', 'Invites Done', 'Invites Done This Wk', 'Invites Done Last Wk', 'Invites Started', 'Started This Wk', 'Started Last Wk', 'From Invites', 'From Commission', 'Commission This Wk', 'Commission Last Wk', 'Created At'].join(','),
        ...allUsers.map(u => [
          '"' + (u.email || 'Anonymous') + '"',
