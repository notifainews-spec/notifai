...
CONFIG
——————————————————— */
const INGEST_MAX_PER_CAT = parseInt(process.env.INGEST_MAX_PER_CAT || "20", 10);
...
const MAX_PER_CATEGORY   = parseInt(process.env.MAX_PER_CATEGORY   || "20", 10);
...
// ⚔️ CONFLICTS: Iran–US war + Ukraine–Russia war
// Covers Iranian state media, neutral Middle East sources, Ukrainian + Western sources
conflicts: [
  ...
  "https://thecradle.co/feed",                             // The Cradle (independent Middle East)
  "https://www.al-monitor.com/rss",                        // Al-Monitor (independent Middle East analysis)
  "https://responsiblestatecraft.org/feed/",               // Responsible Statecraft (independent foreign policy)
  "https://www.france24.com/en/middle-east/rss",           // France 24 Middle East (fast, independent)

  // — Ukraine / Russia war —
  "https://kyivindependent.com/rss/",                      // Kyiv Independent (Ukrainian perspective)
  "https://www.rferl.org/api/zyjpqiumkevit",               // Radio Free Europe / Radio Liberty
  "https://feeds.bbci.co.uk/news/world/europe/rss.xml",    // BBC Europe
  "https://www.reuters.com/world/europe/rss.xml",          // Reuters Europe
  "https://www.theguardian.com/world/ukraine/rss",         // Guardian Ukraine
  "https://www.dw.com/rss/rdf/en-all",                     // Deutsche Welle (fast, independent)
  "https://feeds.skynews.com/feeds/rss/world.xml",         // Sky News World (fast)
  "https://meduza.io/en/rss/all",                          // Meduza (independent Russian/exiled media)
],
};
...
invitesCompletedLastWeek: 0,
invitesStartedThisWeek: 0,
invitesStartedLastWeek: 0,
commissionThisWeek: 0,
commissionLastWeek: 0,
totalSeconds: 0,
weeklySeconds: 0,
};
...
invitesCompletedThisWeek: 0,
invitesStartedLastWeek: data.invitesStartedThisWeek || 0,
invitesStartedThisWeek: 0,
commissionLastWeek: data.commissionThisWeek || 0,
commissionThisWeek: 0,
};
...
inviterUpdate.invitesCompletedThisWeek = admin.firestore.FieldValue.increment(1);
}
if (commission > 0) {
inviterUpdate.tokensFromCommission = admin.firestore.FieldValue.increment(commission);
inviterUpdate.commissionThisWeek = admin.firestore.FieldValue.increment(commission);
}
...
// trackUsageForUser and handleReferralRewards REMOVED — all rewards are ad-based only
...
if (all.find(x => x.url === art.url)) continue;
  ...
  all.push({
    ...
    source: art.source,
    image: art.image,
    category: art.category, // "world" or "crypto" or "conflicts"
...
const out = { us: [], entertainment: [], finance: [], world: [], crypto: [], conflicts: [] };
...
if (a.category === "conflicts") {
if (out.conflicts.length < limit) out.conflicts.push(a);
continue;
}
  ...
  if (!inviterSnap.empty) {
    ...
    await inviterRef.update({
      invitesStarted: admin.firestore.FieldValue.increment(1),
          invitesStartedThisWeek: admin.firestore.FieldValue.increment(1),
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
  }
}
  ...
  while (fetchedCount < maxResults) {
    ...
    for (const doc of snapshot.docs) {
      ...
      users.push({
        ...
        invitesCompletedThisWeek: data.invitesCompletedThisWeek || 0,
        invitesCompletedLastWeek: data.invitesCompletedLastWeek || 0,
        invitesStartedThisWeek: data.invitesStartedThisWeek || 0,
        invitesStartedLastWeek: data.invitesStartedLastWeek || 0,
        commissionThisWeek: data.commissionThisWeek || 0,
        commissionLastWeek: data.commissionLastWeek || 0,
        totalHours: Math.floor((data.totalSeconds || 0) / 3600),
        totalSeconds: data.totalSeconds || 0,
  ...
  <script>
    ...
    async function exportAllCSV() {
      ...
      try {
        ...
        const csv = [
          ['Email', 'Email Verified', 'Wallet', 'Referral Code', 'Invited By', 'Tokens Total', 'From Ads', 'Ads Watched', 'Tokens This Week', 'Tokens Last Week', 'Invites Done', 'Invites Done This Week', 'Invites Done Last Week', 'Invites Started', 'Invites Started This Week', 'Invites Started Last Week', 'From Invites', 'From Commission', 'Commission This Week', 'Commission Last Week', 'Created At'].join(','),
          ...data.users.map(u => [
            '"' + (u.email || 'Anonymous') + '"',
            u.emailVerified ? 'Yes' : 'No',
            u.walletAddress || '-',
            ...
            u.tokensLastWeek,
            u.invitesCompleted,
            u.invitesCompletedThisWeek || 0,
            u.invitesCompletedLastWeek || 0,
            u.invitesStarted || 0,
            u.invitesStartedThisWeek || 0,
            u.invitesStartedLastWeek || 0,
            u.tokensFromInvites || 0,
            u.tokensFromCommission || 0,
            u.commissionThisWeek || 0,
            u.commissionLastWeek || 0,
            u.createdAt || '-'
          ].join(','))
        ].join('\\n');
